...
type 'a list = Empty                        // empty list case
             | NonEmpty of 'a * 'a list     // non-empty list case: rather than an infix operator we use a normal name attacched to a paiur
...
let rec length_of_list2 l =
    ...
    | NonEmpty (x, xs) -> 1 + length_of_list2 xs    // x and xs are bound to the 2 arguments of NonEmpty: x is the head and xs is the tail
